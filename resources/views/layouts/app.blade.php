...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        @yield('title')
        <?php $img = "uploads/settings/" . config('global.favicons') . "_favicons.jpg"; ?>
 ...
 <!-- Import Vue -->
        ...
        <script>
            $('input[accept*=image]').on('change', function(){
                if(this.accept == "image/x-png,image/gif,image/jpeg" && this.value){
                    let splited = this.value.split('.');
                    let extension = splited[splited.length - 1];
                    let availableExtensions = ['png','gif','jpeg','jpg'];
                    if(!availableExtensions.includes(extension)){
                        callModalMessage('{{ __('This image format is not supported. Please only send images in png, jpg, jpeg or gif format') }}', '{{ __('Unsupported format') }}');
                        this.value = null;
                    }
                }
            });
