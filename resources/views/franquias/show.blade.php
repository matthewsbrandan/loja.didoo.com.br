...
<section class="custon-banner section-profile-cover section-shaped grayscale-05 d-lg-block d-lx-block">
    <!-- Circles background -->
    <img class="bg-image" loading="lazy" src="{{ $restorant->coverm }}" style="width: 100%;">
    <!-- SVG separator -->
    <div class="checkIsopen">
        <span
            style="{{!empty($openingTime) && !empty($closingTime) ? 'background-color: #d9d375 ': 'background-color: red ' }}"
        >
...
<section class="section pt-lg-0 pb-0 pb-lg-6" id="restaurant-content" style="padding-top: 0px">
    ...
    <div class="container container-restorant">
        ...
        <div id="{{ clean(str_replace(' ', '', strtolower($category->name)).strval($key)) }}" class="{{ clean(str_replace(' ', '', strtolower($category->name)).strval($key)) }}">
            <h1 class="titleCategory" >{{ $category->name }}</h1><br />
        </div>
        @endif
        <div class="row  {{ clean(str_replace(' ', '', strtolower($category->name)).strval($key)) }}">
            ...
            <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6 handle-search-product" data-title="{{ $item->name }}" id="product-{{ $item->id }}">
                <div class="strip productContainer" onClick="setCurrentItem({{ $item->id }})">
                    @if(!empty($item->image))
                        <figure >
                    ...
                    <div class="productInfo">
                        <span class="res_title"><b><a onClick="setCurrentItem({{ $item->id }})" href="javascript:void(0)">{{ $item->name }}</a></b></span>
                        <span class="res_description">{{ $item->short_description}}</span>
