...
<tr>
    ...
    <td class="table-web">
        {{  $order->created_at->setTimezone('America/Sao_Paulo')->format(config('settings.datetime_display_format')) }}
    </td>
    <td class="table-web">
        @if( $order->table)
            {{  $order->table->restoarea?$order->table->restoarea->name." - ".$order->table->name:$order->table->name }}
        @else
            @if($order->delivery_method = 4 && $order->table_number)
                {{ __('Table'). ' ' . $order->table_number }}
            @else
                {{ __('Takeaway')}}
            @endif
        @endif
