...
class Items extends TranslateAwareModel
{
    ...
    protected $fillable = ['name', 'description', 'image', 'image_variations', 'price', 'category_id', 'vat'];
    ...
    public function getImageVariations(){
        if($this->image_variations != null){
            $arrImages = explode(',',$this->image_variations);
            $arrFormatted = [];
            foreach($arrImages as $image){
                $pre = $this->getImge($image, config('global.restorant_details_image'));
                $pre = str_replace('restorants/','restorants/variations/',$pre);
                $arrFormatted[]=$pre;
            }
            return $arrFormatted;
        }
