...
function updatePrices(net,delivery,enableDelivery){
  ...
  if(enableDelivery){
    ...
    if($('#deliveryToMatch') && $('#deliveryToMatch').val() === 'true') cartTotal.deliveryPriceFormated= "À combinar";
    else cartTotal.deliveryPriceFormated=formatter.format(delivery);
  ...
  handleTagCart(cartTotal.totalPriceFormat);
}

function handleTagCart(total){
  if($('#price-here') && $('#qtd-here') &&  $('#infoCartCuston') ){
    $('#price-here').html(total);
    let itemsArray = Object.entries(cartContent.items).map((arr) => arr);
    $('#qtd-here').html(itemsArray.length);
...
function updateSubTotalPrice(net,enableDelivery){
  let delivery = Number($("#deliveryCost").val()) ?? (cartTotal.deliveryPrice?cartTotal.deliveryPrice:0);
  updatePrices(net,delivery,enableDelivery, true)
}
/**
...
function orderTypeSwither(mod){
      ...
      $('.picTime').hide();
      $('#tableNumber').hide();
      $('#card-table-delivery-taxes').hide();
      ...
      if(mod=="delivery"){
          $('.delTime').show();
          $('#addressBox').show();
          $('#card-table-delivery-taxes').show();
          getCartContentAndTotalPrice();
      }
}
...
function chageDeliveryCost(deliveryCost, hasDelivery = true){
  $("#deliveryCost").val(deliveryCost);
  updatePrices(cartTotal.totalPrice,deliveryCost,hasDelivery);
