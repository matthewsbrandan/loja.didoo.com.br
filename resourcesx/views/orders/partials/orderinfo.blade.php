    ...
    <h6 class="heading-small text-muted mb-4">{{ __('Restaurant information') }}</h6>
     ...
     @if(!empty($order->comment))
        ...
        <?php
            ...
            $comment = str_replace("<br/><br/><br/>","<br/>",$comment);
            $comment = str_replace("<br/><br/>","<br/>",$comment);
        ?>
        <h4>{{ __('Comment') }}:<br/> {!! $comment !!}</h4>
     @endif
     ...
     @if(config('app.isft'))
         ...
         @if(!strpos($order->comment,'*'.__('Scheduled to').'*'))
            <h3>{{ __("Time slot") }}: @include('orders.partials.time', ['time'=>$order->time_formated])</h3>
         @endif
     @else
         <h4>{{ __("Delivery method") }}: 

            @if($order->delivery_method==1) {{ __('Delivery') }}
            @elseif($order->delivery_method==2) {{ __('Pickup') }}
            @elseif($order->delivery_method==3) {{ __('Deliver in') }}
            @elseif($order->delivery_method==4) {{ __('Table') . ' ' . $order->table_number }}
            @else {{ __('Takeaway') }} @endif
        </h4>
         @if ($order->delivery_method!=3 && !strpos($order->comment,'*'.__('Scheduled to').'*'))
             <h3>{{ __("Time slot") }}: @include('orders.partials.time', ['time'=>$order->time_formated])</h3>
         @endif
